...
Item {
  ...
  // User-owned floating bar settings, in logical pixels.
  ...
  function windowScreenPoint(scenePoint, window) {
    ...
    // Scene coordinates are local to the floating panel, not the screen edge.
    x += root.position === "right"
      ? window.screen.width - window.width - window.margins.right
      : window.margins.left
    y += root.position === "bottom"
      ? window.screen.height - window.height - window.margins.bottom
      : window.margins.top
  ...
  function applyBarConfig() {
    ...
    outerMargin = Number.isFinite(Number(config.outerMargin)) ? Math.max(0, Math.round(Number(config.outerMargin))) : 0
    cornerRadius = Number.isFinite(Number(config.cornerRadius)) ? Math.max(0, Math.round(Number(config.cornerRadius))) : 0
    frameWidth = Number.isFinite(Number(config.frameWidth)) ? Math.max(0, Math.round(Number(config.frameWidth))) : 6
    frameRadius = Number.isFinite(Number(config.frameRadius)) ? Math.max(0, Math.round(Number(config.frameRadius))) : 20
  ...
  // The left rail joins a thin desktop frame. Its rounded opening produces
  // inward-facing corners without painting over the bar's interactive widgets.
  Variants {
    model: Quickshell.screens
    delegate: Component {
      DesktopFrame {
  ...
  component DesktopFrame: PanelWindow {
    id: frameWindow
    visible: root.position === "left" && !root.barHidden && !root.transparent
    color: "transparent"
    surfaceFormat.opaque: false
    exclusionMode: ExclusionMode.Ignore
    WlrLayershell.namespace: "sym-desktop-frame"
    WlrLayershell.layer: WlrLayer.Top
    WlrLayershell.keyboardFocus: WlrKeyboardFocus.None
    anchors { top: true; bottom: true; left: true; right: true }
    // Empty input region: the decorative surface never intercepts clicks.
    mask: Region {}

    Canvas {
      id: frameCanvas
      anchors.fill: parent
      property color frameColor: root.background
      property int railWidth: root.barSize
      property int borderWidth: root.frameWidth
      property int openingRadius: root.frameRadius
      onFrameColorChanged: requestPaint()
      onRailWidthChanged: requestPaint()
      onBorderWidthChanged: requestPaint()
      onOpeningRadiusChanged: requestPaint()
      ...
      onPaint: {
        ...
        var left = Math.min(railWidth, width)
        var top = Math.min(borderWidth, height / 2)
        var right = Math.max(left, width - borderWidth)
        var bottom = Math.max(top, height - borderWidth)
        var radius = Math.max(0, Math.min(openingRadius, (right - left) / 2, (bottom - top) / 2))
        ctx.fillStyle = frameColor
        ctx.fillRect(left, 0, width - left, height)
        // Remove a rounded opening; the remaining corner pieces curve inward.
  ...
  component BarPanel: PanelWindow {
    ...
    margins {
      top: root.barHidden && root.position === "top" ? -root.barSize : root.outerMargin
      bottom: root.barHidden && root.position === "bottom" ? -root.barSize : root.outerMargin
      left: root.barHidden && root.position === "left" ? -root.barSize : root.outerMargin
      right: root.barHidden && root.position === "right" ? -root.barSize : root.outerMargin
    }
